...
namespace DB_project1
{
    class Controller
    {
        ...
        /*--------------------------------------------------------------------------------------------*/
        public int Insert_Ad_Contract(DateTime start, DateTime end, string cname, string nid, string adress, string pid, string phoneNum, int ad_bus_id)
        {
            string query;
            query = "insert into AD_Contract (StartDate,EndDate,Client_Name,Client_NationalID,Client_Address,Package_ID,Client_Phone,AD_Bus_ID) " +
                " values('" + start + "','" + end + "','" + cname + "','" + nid + "','" + adress + "','" + pid + "','" + phoneNum + "'," + ad_bus_id + "); ";
            return dbMan.ExecuteNonQuery(query);
        }
